...
test('Every personal table scopes reads and cannot transfer rows to another user',async()=>{
 const requests=[];const {c,storage}=setup(async(url,options)=>{requests.push({url,options});return Response.json([])});storage.set('dv_os_expires_at',String(Date.now()+3600000));vm.runInContext("currentUser={id:'user-a'}",c);
 for(const name of ['events','career_entries','wishlist_items','expenses']){await c.tableRequest('https://example.test/rest/v1/'+name,'?user_id=eq.user-b');assert.equal(new URL(requests.at(-1).url).searchParams.get('user_id'),'eq.user-a');await c.tableRequest('https://example.test/rest/v1/'+name,'?id=eq.record',{method:'PATCH',body:JSON.stringify({user_id:'user-b'})});assert.equal(JSON.parse(requests.at(-1).options.body).user_id,'user-a');}
});
