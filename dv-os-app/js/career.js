...
async function loadCareer() {
  reportModuleLoad('percorso','loading');
  showSkeleton(careerMenu, 3);
  try {
    careerEntries = await careerRequest('?select=*&order=sort_order.asc,created_at.asc', { method: 'GET' });
    renderCareer();
    reportModuleLoad('percorso','ok');
  } catch (err) {
    reportModuleLoad('percorso','error',err.message);
    console.error('Errore nel caricamento del percorso:', err);
    showLoadError(careerMenu, loadCareer, err.message);
  }
}

function renderCareer() {
  clearSkeleton(careerMenu);
