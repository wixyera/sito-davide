...
const MODULE_TITLES = {
  paypal: 'Payment Studio', home: 'Panoramica', calendario: 'Calendario', percorso: 'Percorso', contatti: 'Contatti',
...
function showModule(name, options = {}) {
  ...
  const canAnimate = options.transition && !window.matchMedia('(prefers-reduced-motion:reduce)').matches && !document.body.classList.contains('motion-paused') && !document.getElementById('appShell').inert;
