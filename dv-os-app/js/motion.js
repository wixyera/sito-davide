...
(function(){
  ...
  /* ---------------- REVEAL: vero scroll-reveal, non solo al cambio modulo ----------------
     PRIMA: al cambio scheda tutti gli elementi [data-reveal] del modulo comparivano
     insieme, anche quelli più in basso fuori dallo schermo — risultato: scorrendo la
     pagina non succedeva più nulla, tutto era già apparso in un colpo solo.
     ORA: un IntersectionObserver anima ogni blocco quando entra DAVVERO nel viewport,
     sia scorrendo sia al cambio modulo. In più tagghiamo automaticamente altri blocchi
     "a container" (pannelli, griglie, form) così anche le sezioni più lunghe hanno
     qualcosa che si anima mentre si scorre, senza dover mettere data-reveal a mano
     ovunque nell'HTML. */
  const AUTO_REVEAL_SELECTOR = '.cal-panel, .console > *, .wl-summary-box, .sp-cat-breakdown, .wl-grid, .footer-cta';
  ...
  function observeReveals(root){
    ...
    if (revealIO) items.forEach(el => revealIO.observe(el));
    else items.forEach(el => el.classList.add('is-in')); // reduced motion / niente IO: mostra e basta
  }
  observeReveals();

  const mainEl = document.querySelector('main');
  if (mainEl && revealIO) {
    // al cambio modulo gli elementi tornano "non rivelati": così l'animazione riparte
    // ogni volta che si riapre una sezione, sia sopra sia sotto la piega.
  ...
  const MAGNETIC_SELECTOR = '.tab-btn, .theme-toggle-btn, .go, .ev-add-btn, .export-all-btn, .nav-toggle, .quick-card .go, .hero-explore, .go-btn, .carousel-arrow, #searchTriggerBtn';
  if (!reducedMotion && isFinePointer) {
  ...
  /* ---------------- EFFETTO "ACQUA" SULL'HERO ----------------
     NOTA PERFORMANCE: prima questo loop aggiornava feTurbulence/
     feDisplacementMap ad ogni frame (60/s), per SEMPRE, anche senza
     mouse (mobile) e anche a tab in background. Il filtro SVG e'
     condiviso da tutte le hero-water a schermo intero, quindi ogni
     tick ricalcolava un filtro pesante su immagini enormi: la causa
     principale degli scatti su telefono. Ora: su pointer coarse
     (mobile/touch) il filtro resta statico (nessun loop), su desktop
     si aggiorna a ~15fps invece di 60fps, e si ferma del tutto se la
     scheda e' in background o se l'utente preferisce motion ridotto. */
  ...
  if (heroVisual && turb && disp) {
    ...
    let t = 0;
    let waterRunning = true;
    const STEP_MS = 65; // ~15fps: sufficiente per un "respiro" morbido, molto piu' leggero di 60fps

    function tickWater(){
      t += 0.03;
      const idleFreq = 0.012 + Math.sin(t) * 0.004;
      turb.setAttribute('baseFrequency', idleFreq.toFixed(4) + ' ' + (idleFreq * 1.6).toFixed(4));
      curScale += (targetScale - curScale) * 0.3;
      disp.setAttribute('scale', curScale.toFixed(1));
    }

    if (reducedMotion) {
      // nessuna animazione: un solo valore statico, filtro calcolato una volta sola
      turb.setAttribute('baseFrequency', '0.0120 0.0192');
      disp.setAttribute('scale', '10');
    } else if (!isFinePointer) {
      // mobile/touch: niente loop, il filtro resta statico (zero costo per-frame)
      turb.setAttribute('baseFrequency', '0.0120 0.0192');
      disp.setAttribute('scale', '10');
    } else {
      let waterTimer = setInterval(() => { if (waterRunning) tickWater(); }, STEP_MS);
  ...
  /* ---------------- SCROLL: barra progresso, header "vetro", parallax hero, torna su ----------------
     Un solo listener con throttling a requestAnimationFrame per tutti gli effetti legati
     allo scroll (prima c'era solo la barra di progresso): così scorrere la pagina si sente
     vivo — header che si "vetrifica" appena ci si allontana dalla cima, foto hero che si
     muove leggermente più lenta del resto (parallax), pulsante per tornare su che compare
     dopo un po' di scroll — invece di stare fermo fino al prossimo cambio scheda. */
  ...
  function updateOnScroll(){
    ...
    document.body.classList.toggle('scrolled', scrolled > 40);

    if (backToTop) backToTop.classList.toggle('show', scrolled > 500);

    if (!reducedMotion) {
      const activeMedia = document.querySelector('.module.active .page-hero-media');
      if (activeMedia) {
        const y = Math.min(scrolled * 0.18, 60);
        activeMedia.style.transform = `translate3d(0, ${y}px, 0)`;
      }
    }
    scrollTicking = false;
  }
