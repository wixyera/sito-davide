...
(()=>{
 ...
 story.innerHTML='<figure><img src="assets/editorial/runway.jpg" alt="Silhouette sartoriale nera in uno spazio architettonico con tende bordeaux" width="1536" height="1024" loading="lazy"><figcaption>THE EDITORIAL / 01</figcaption></figure><div><span class="eyebrow">A COLLECTION OF YOU</span><h2>Il dettaglio<br>fa la <em>differenza.</em></h2><p>Ciò che ti ispira. Quello che desideri. Le idee da non lasciare a domani. Una collezione che parla di te.</p><button type="button" class="primary-button">Sfoglia la wishlist ↗</button></div>';
 ...
 function sync(){if(paused())running.forEach(a=>a.cancel());document.querySelectorAll('.runway-video').forEach(v=>{if(paused()||document.hidden||!v.closest('dialog').open)v.pause()})}
 query.addEventListener('change',sync);addEventListener('space:motion',sync);document.addEventListener('visibilitychange',sync);addEventListener('pagehide',()=>{observer?.disconnect();accessObserver.disconnect();running.forEach(a=>a.cancel())});
})();
