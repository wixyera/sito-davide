...
async function updateSecurityQuestion() {
  ...
  if (!question) return toastError('Scelta non valida.');
  const answer = prompt(`Domanda scelta: "${question}"\nScrivi la risposta segreta (la ricorderai per il reset password):`);
  if (!answer || !answer.trim()) return toastError('Risposta non valida, operazione annullata.');
  try {
    ...
    currentUser = null;
    toastSuccess('Domanda di sicurezza salvata! Potrai usarla per reimpostare la password senza email.');
  } catch (e) {
    toastError('Errore nel salvataggio: ' + e.message);
  }
}
