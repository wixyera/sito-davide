...
const files=['index.html','paypal-sandbox.html','privacy.html','manifest.json','service-worker.js','robots.txt','_headers','assets','css','js','experiments','vendor'];
...
for(const api of ['oracolo','spotify-search','fetch-product','football-news','product-image','paypal-config','paypal-order'])await access(path.join(root,'functions/api',api+'.js'));
