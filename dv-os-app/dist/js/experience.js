...
(() => {
  ...
  try { motionPaused = motionPaused || readPersonalPreference('motion') === 'paused'; } catch (_) {}
  let galleryApi = null;
  ...
  for (const id of ['welcomeMotion', 'motionToggle']) $(id).addEventListener('click', () => {
    ...
    try { writePersonalPreference('motion', motionPaused ? 'paused' : 'active'); } catch (_) {}
  });
  motionQuery.addEventListener('change', () => {
    let preference = false;
    try { preference = readPersonalPreference('motion') === 'paused'; } catch (_) {}
    setMotion(preference);
  });
  window.addEventListener('personal:appearance',event=>setMotion(event.detail.motion==='paused'));
  setMotion(motionPaused);
