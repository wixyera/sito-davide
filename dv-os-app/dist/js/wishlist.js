...
async function loadWishlist() {
  reportModuleLoad('wishlist','loading');
  showSkeleton(wlGrid, 3);
  try {
    wishlistEntries = await wishlistRequest('?select=*&order=created_at.desc', { method: 'GET' });
    renderWishlist();
    reportModuleLoad('wishlist','ok');
  } catch (err) {
    reportModuleLoad('wishlist','error',err.message);
...
function renderWishlist() {
  ...
  const filter=document.getElementById('wishFilter')?.value||'all';
  const query=(document.getElementById('wishSearch')?.value||'').trim().toLocaleLowerCase('it');
  const visible=wishlistEntries.filter(item=>{const d=wishlistDetails(item.notes);return (filter==='all'||filter==='purchased'&&d.purchased||filter==='waiting'&&!d.purchased||filter==='high'&&d.priority==='high')&&[item.title,item.site_name,item.category,d.color].join(' ').toLocaleLowerCase('it').includes(query)});
  wlEmpty.style.display=visible.length?'none':'block';
  wlEmpty.textContent=wishlistEntries.length?'Nessun pezzo corrisponde alla ricerca. Prova un altro filtro.':'La tua selezione comincia da un desiderio. Aggiungi il primo pezzo dal modulo.';
  visible.forEach(item => {
    ...
    const card = document.createElement('div');card.dataset.recordId=item.id;
    ...
    const details=wishlistDetails(item.notes), detailLine=document.createElement('p');detailLine.className='wish-spec';detailLine.textContent=[details.size&&'Taglia '+details.size,details.color,details.priority==='high'?'★ Priorità alta':details.priority==='low'?'Per più avanti':'',details.purchased?'✓ Acquistato':''].filter(Boolean).join(' · ');card.querySelector('.wl-body').insertBefore(detailLine,card.querySelector('.wl-actions'));card.classList.toggle('is-purchased',details.purchased);
    ...
    card.querySelector('.ev-del').onclick = async () => {
      const button=card.querySelector('.ev-del');button.disabled=true;
      if (!await confirmWishlistRemoval(item)){button.disabled=false;return}
      try {
        ...
        await loadWishlist();
      } catch (err) { toastError('Errore durante l\u2019eliminazione: ' + err.message); } finally {button.disabled=false}
    };
...
function editWishlistItem(item) {
  ...
  const d=wishlistDetails(item.notes);document.getElementById('wlNotes').value=d.notes;document.getElementById('wlSize').value=d.size;document.getElementById('wlColor').value=d.color;document.getElementById('wlPriority').value=d.priority;document.getElementById('wlPurchased').value=String(d.purchased);
  wlUrlInput.value = '';
...
wlForm.addEventListener('submit', async e => {
  ...
  const payload = {
    title,
    price: priceRaw ? (parseMoneyCents(priceRaw)===null?NaN:parseMoneyCents(priceRaw)/100) : null,
    ...
    notes: packWishlistDetails({notes:document.getElementById('wlNotes').value.trim(),size:document.getElementById('wlSize').value.trim(),color:document.getElementById('wlColor').value.trim(),priority:document.getElementById('wlPriority').value,purchased:document.getElementById('wlPurchased').value==='true'}),
  ...
  try {
    ...
    if(typeof clearWorkspaceDraft==='function')clearWorkspaceDraft('wlForm');
    resetWishlistForm();
    await loadWishlist();
    toastSuccess('La tua selezione è stata salvata.');
  } catch (err) {
