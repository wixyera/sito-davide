...
function buildSearchIndex() {
  ...
  if(typeof cleanSavedOutfits==='function')for(const look of cleanSavedOutfits(currentUser?.user_metadata?.saved_outfits))idx.push({module:'outfit',id:look.id,title:look.name,sub:'Outfit salvato',haystack:norm([look.name,...Object.values(look.items).map(x=>x.title)].join(' '))});
  if(typeof savedArticles==='function')for(const article of savedArticles())idx.push({module:'calcio',id:article.url,title:article.title,sub:article.source+' · '+article.team,url:article.url,haystack:norm([article.title,article.source,article.team].join(' '))});
  return idx;
}
...
function renderSearchResults(query) {
  ...
  const matches = idx.filter(r => q.split(/\s+/).every(word=>r.haystack.includes(word))).slice(0, 40);

  searchEmptyEl.style.display = matches.length ? 'none' : 'block';
  const statusEl = document.getElementById('searchStatus');
  if (statusEl) statusEl.textContent = matches.length ? `${matches.length} risultat${matches.length === 1 ? 'o' : 'i'} trovat${matches.length === 1 ? 'o' : 'i'}.` : 'Nessun risultato.';
  ...
  Object.keys(groups).forEach(mod => {
    ...
    groupEl.innerHTML = `<div class="search-group-label">${SEARCH_ICONS[mod]||SEARCH_ICONS.wishlist}<span></span></div>`;
    groupEl.querySelector('span').textContent = SEARCH_LABELS[mod]||({outfit:'Outfit',calcio:'Articoli salvati'}[mod]);
...
function goToSearchResult(m) {
  closeSearch();
  if(m.url){const a=document.createElement('a');a.href=m.url;a.target='_blank';a.rel='noopener noreferrer';a.click();return;}
  showModule(m.module);
  if(m.module==='wishlist'){const x=wishlistEntries.find(x=>x.id===m.id);if(x)editWishlistItem(x)}
  if(m.module==='spese'){const x=expenseEntries.find(x=>x.id===m.id);if(x){const d=new Date(x.expense_date+'T12:00:00');speseViewDate=new Date(d.getFullYear(),d.getMonth(),1);document.getElementById('spSearch').value='';document.getElementById('spCategoryFilter').value='';renderExpenses();editExpense(x)}}
  if(m.module==='percorso'){const x=careerEntries.find(x=>x.id===m.id);if(x)editCareer(x)}
  if(m.module==='contatti'){const x=contactEntries.find(x=>x.id===m.id);if(x)editContact(x)}
  if(m.module==='outfit')window.dispatchEvent(new CustomEvent('workspace:open-outfit',{detail:m.id}));
  if (m.module === 'calendario' && m.dateKey) {
    const [y, mo, d] = m.dateKey.split('-').map(Number);
    viewYear = y; viewMonth = mo - 1;
    selectDay(m.dateKey, d);const event=(events[m.dateKey]||[]).find(x=>x.id===m.id);if(event)editEvent(event);
  }
}
...
/* Focus-trap: mentre il pannello è aperto, Tab/Shift+Tab restano dentro
   il pannello invece di uscire verso il resto della pagina sottostante. */
searchOverlay.addEventListener('keydown', e => {
  if (e.key !== 'Tab') return;
  const focusables = searchOverlay.querySelectorAll('input, button, [href], [tabindex]:not([tabindex="-1"])');
...
searchOverlay.addEventListener('keydown',e=>{if(!['ArrowDown','ArrowUp'].includes(e.key))return;const rows=[...searchResultsEl.querySelectorAll('.search-result')];if(!rows.length)return;e.preventDefault();const i=rows.indexOf(document.activeElement);rows[(i+(e.key==='ArrowDown'?1:-1)+rows.length)%rows.length].focus()});
