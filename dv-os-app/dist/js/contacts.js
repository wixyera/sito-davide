...
async function loadContacts() {
  reportModuleLoad('contatti','loading');
  showSkeleton(contactList, 3);
  try {
    contactEntries = await contactsRequest('?select=*&order=updated_at.desc', { method: 'GET' });
    renderContacts();
    reportModuleLoad('contatti','ok');
  } catch (err) {
    reportModuleLoad('contatti','error',err.message);
    console.error('Errore nel caricamento dei contatti:', err);
    showLoadError(contactList, loadContacts, err.message);
  }
}

function renderContacts() {
  clearSkeleton(contactList);
