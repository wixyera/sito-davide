...
export function publicProductUrl(value) {
  const u = new URL(value), host = u.hostname.toLowerCase();
  if (!['https:', 'http:'].includes(u.protocol) || u.username || u.password || (u.port && !['80','443'].includes(u.port)) || !host.includes('.') || /^[\d.]+$/.test(host) || host.includes(':') || /(?:^|\.)(?:localhost|local|internal|test|invalid)$/.test(host)) throw Error('Usa il link pubblico del negozio.');
  return u;
}
...
export async function onRequestPost(context) {
  ...
  let target;
  try {
    target = publicProductUrl(rawUrl);
  ...
  try {
    ...
    if (!location || hop === 4) throw Error('Troppi reindirizzamenti');
