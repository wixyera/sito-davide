...
document.getElementById('exportAllBtn').onclick = () => {
  ...
  if (!list.length) { alert('Nessun evento da esportare in questo mese.'); return; }
...
document.getElementById('importICSInput').addEventListener('change', async e => {
  ...
  try {
    ...
    if (!payload.length) { alert('Nessun evento valido trovato nel file.'); return; }
    await supabaseRequest('', { method: 'POST', body: JSON.stringify(payload) });
    await loadEvents();
    alert(`${payload.length} eventi importati.`);
  } catch (err) {
